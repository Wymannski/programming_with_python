...
class Game:
    ...
    def int_to_bin_mod8(self, num_matches: int) -> list[bool]:
        ...
        """
        dictionary = {0: [False, False, False], 1: [False, False, True], 2: [False, True, False],
                      3: [False, True, True],
                      4: [True, False, False],
                      5: [True, False, True], 6: [True, True, False], 7: [True, True, True]}
        num_matches %= 8
        return dictionary[num_matches]
    ...
    # add your code here
    def transpose(self,items: list[list]) -> list[list]:
        if not items or not items[0]:
            return []
        return [list(map(lambda a: a[0], items))] + self.transpose(list(map(lambda b: b[1:], items)))
    ...
    def is_winning(self):
        ...
        #     intermediate_val = row[0]
        #     for val in row[1:]:
        #         intermediate_val = intermediate_val ^ val
        #     result.append(intermediate_val)
        # return True in result
        return reduce(lambda c,d: c or d ,map(lambda x: reduce(lambda a,b: a ^ b,x) ,self.row_to_col(self.game_to_bin())))
