...
The Game is represented by a list of integers and the Binary conversion is done with a 
...
class Game:
    ...
    def int_to_bin_mod8(self, num_matches: int) -> list[bool]:
        ...
        return dictionary[num_matches%8]
    ...
    def game_to_bin(self) -> list[list[bool]]:
        ...
        it means that a list of lists of booleans is provided (each element of the list
        is a row converted into binary).
        :return: a list of lists of booleans
        """
        result = []
        for item in self.game:
            result.append(self.int_to_bin_mod8(item))
        return result
    ...
    def row_to_col(self, lst_lst: list[list[Any]]) -> list[list[Any]]:
        ...
        This implementation takes advantage of map function and recursion and which makes
        the implementation easier to develop and understand.
        :param lst_lst: the list of lists (row - col) to transform
        :return: the transformed list of lists (col - row)
        """
        # result = []
        # for index_col in range(len(lst_lst[0])):
        #     transposed_row = []
        #     for row in lst_lst:
        #         transposed_row.append(row[index_col])
        #     result.append(transposed_row)
        # return result
        # if len(lst_lst) == 0 or len(lst_lst[0]) == 0:
        #     return []
        #
        # return [list(map(list,lst_lst[0])), list(map(list,self.row_to_col(lst_lst[1:])))]
        return list(map(list,zip(*lst_lst)))
    ...
    def is_winning(self):
        ...
        """
        # result = []
        # for row in self.row_to_col(self.game_to_bin()):
        #     intermediate_item = row[0]
        #     for item in row[1:]:
        #        intermediate_item = intermediate_item ^ item
        #     result.append(intermediate_item)
        # return True in result
        #test = zip(*self.row_to_col(self.game_to_bin()))
        return reduce(lambda x,y: x or y,map(lambda x: reduce(lambda a,b: a ^ b,x),self.row_to_col(self.game_to_bin())))
